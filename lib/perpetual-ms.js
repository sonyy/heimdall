...
const { fetchKlines, fetchKlinesRange, fetchCandles, VALID_TIMEFRAMES, tfToMinutes, normalizeTf } = require('./exchange');
  ...
  if (msgId) {
    try {
      ...
      const fresh = await bot.sendMessage(chatId, text, opts).catch(e2 => {
        console.error('PERP sendMenu edit+send err:', e.message, '/', e2.message);
        return null;
      });
      return fresh;
    }
    ...
    try { return await bot.sendMessage(chatId, text, opts); } catch (e) {
      console.error('PERP sendMenu send err:', e.message);
    }
  }
}
...
function showPerpFeatureMenu(bot, chatId, msgId) {
  ...
  const tp1 = getFeatConfig('perp', 'tp1Percent', '2');
  const tp2 = getFeatConfig('perp', 'tp2Percent', '4');
    ...
    parse_mode: 'HTML',
      reply_markup: {
        inline_keyboard: [
          ...
          [{ text: '\u2795 Add/Edit Pair', callback_data: 'perp_managepair' }],
...
async function showPerpStatus(bot, chatId, msgId) {
  try {
    const running = getFeatConfig('perp', 'running', '0') === '1';
    const waitMode = getFeatConfig('perp', 'waitMode', 'trend');
    const lookback = getFeatConfig('perp', 'swingLookback', '2');
...
module.exports = {
  register(bot) {
      ...
      try {
        if (session.cmd === 'perp_managepair') {
          if (session.step === 'ticker') {
            ...
            const exists = db.prepare('SELECT COUNT(*) as c FROM perp_pairs WHERE ticker = ?').get(ticker).c > 0;
          ...
          if (session.step === 'timeframes') {
            ...
            db.transaction(() => {
              if (exists) db.prepare('DELETE FROM perp_pairs WHERE ticker = ?').run(ticker);
              const ins = db.prepare('INSERT OR IGNORE INTO perp_pairs (ticker, timeframe) VALUES (?, ?)');
    ...
    function handleCallback(query) {
      ...
      try {
        if (data === 'perp_status') {
          showPerpStatus(bot, chatId, msgId);
          return { action: null };
        }
        if (data === 'perp_config') {
        ...
        if (data === 'perp_managepair') {
          ...
          if (keys.length) {
            ...
            const rows = keys.map(t => [{ text: `📝 ${t}`, callback_data: `perp_managepair_edit_${t}` }]);
            rows.push([{ text: '➕ Add New', callback_data: 'perp_managepair_new' }]);
            rows.push([{ text: '🔙 Back', callback_data: 'perp_config_back' }]);
            sendMenu(bot, chatId, msgId, `Pair terdaftar:\n${lines.join('\n')}\n\nPilih pair untuk diedit, atau Add New untuk menambah:`, {
        ...
        if (data === 'perp_managepair_new') {
          conv[chatId] = { cmd: 'perp_managepair', step: 'ticker', data: {} };
          bot.sendMessage(chatId, 'Masukkan ticker (contoh: BTCUSDT):');
          return { action: null };
        }
        if (data.startsWith('perp_managepair_edit_')) {
          const ticker = data.replace('perp_managepair_edit_', '');
          const exists = db.prepare('SELECT COUNT(*) as c FROM perp_pairs WHERE ticker = ?').get(ticker).c > 0;
          if (exists) {
            conv[chatId] = { cmd: 'perp_managepair', step: 'timeframes', data: { ticker, exists: true } };
            bot.sendMessage(chatId, `Ticker: ${ticker}\nMasukkan timeframe BARU (pisahkan koma).\nValid: <code>${VALID_TIMEFRAMES.join(', ')}</code>`, { parse_mode: 'HTML' });
          } else {
            conv[chatId] = { cmd: 'perp_managepair', step: 'timeframes', data: { ticker, exists: false } };
            bot.sendMessage(chatId, `Ticker: ${ticker}\nMasukkan timeframe (pisahkan koma).\nValid: <code>${VALID_TIMEFRAMES.join(', ')}</code>`, { parse_mode: 'HTML' });
          }
          return { action: null };
        }
        ...
        if (data === 'perp_config_waitmode') {
          ...
          sendMenu(bot, chatId, msgId, `Mode saat ini: ${cur}\nPilih mode konfirmasi:`, {
            reply_markup: {
              inline_keyboard: [
...
conv[chatId] = { cmd: 'perp_config', step: 'swingLookback', data: {} };
        ...
        if (data.startsWith('perp_managepair_remove_')) {
          ...
          try {
            db.prepare('DELETE FROM perp_pairs WHERE ticker = ?').run(ticker);
            sendMenu(bot, chatId, msgId, `\u2705 ${ticker} dihapus.`, {
              reply_markup: { inline_keyboard: [[{ text: '\ud83d\udd19 Back', callback_data: 'perp_config_back' }]] }
            });
          } catch (e) {
