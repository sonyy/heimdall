...
function formatSummaryMessage() {
  ...
  const marginPct = getFeatConfig('bt', 'marginPercent', '0');
  const isCompoundMode = Number(marginPct) > 0;
  const waitTxt = indicator === 'ms' ? ` | Wait: ${waitMode}` : '';

  const lines = ['<b>Backtest Supertrend</b>\n'];
  const marginLabel = isCompoundMode ? `${marginPct}% compound` : `$${Number(usdtPerTrade).toLocaleString()}/trade`;
  lines.push(`⚙️ SL: ${sl}% | TP: ${tp1}% | ${indicator.toUpperCase()}${waitTxt} | ST ${period}×${multiplier} | 💰 $${Number(startCapital).toLocaleString()} / 💵 ${marginLabel}\n`);
  if (pairKeys.length) {
...
async function runBacktest(ticker, timeframe, bot, chatId, msgId) {
  ...
  const btMarginPct = Number(getFeatConfig('bt', 'marginPercent', '0'));
  const isCompound = btMarginPct > 0;
    ...
    const batchResults = await Promise.all(batch.map(async ({ ticker: t, tf }) => {
      try {
            ...
            if (openTrade) {
              ...
              if (openTrade.direction === 'LONG' && lowPrice <= liq) {
                ...
                if (lowPrice <= openTrade.sl) {
                  ...
                  const pnl = calcPnl('LONG', openTrade.entry, cp, leverage);
                  const cx = capital + (pnl / 100) * m;
                  insTrade.run(t, tf, openTrade.entry, cp, pnl.toFixed(2), liq.toFixed(2), FEE_PCT, openTrade.sl, openTrade.tp1, null, 'LOSE', 'LONG', openTrade.openAt, ts, m, ce, cx);
                  trades.push({ pnl, result: 'LOSE', direction: 'LONG' }); capital += (pnl / 100) * m;
                  closed = true;
                } else if (highPrice >= openTrade.tp1) {
                  const cp = openTrade.tp1;
                ...
                if (highPrice >= openTrade.sl) {
                  ...
                  const pnl = calcPnl('SHORT', openTrade.entry, cp, leverage);
                  const cx = capital + (pnl / 100) * m;
                  insTrade.run(t, tf, openTrade.entry, cp, pnl.toFixed(2), liq.toFixed(2), FEE_PCT, openTrade.sl, openTrade.tp1, null, 'LOSE', 'SHORT', openTrade.openAt, ts, m, ce, cx);
                  trades.push({ pnl, result: 'LOSE', direction: 'SHORT' }); capital += (pnl / 100) * m;
                  closed = true;
                } else if (lowPrice <= openTrade.tp1) {
                  const cp = openTrade.tp1;
              ...
              if (signal.direction === 'LONG') {
                ...
                if (btWaitMode === 'trend') {
                  ...
                  if ((isCompound ? capital > 0 : capital >= btUsdtPerTrade) && signal.retested && price > signal.level) {
                    openTrade = { entry: price, sl: price * (1 + (btSlPct / leverage) / 100), tp1: price * (1 + (btTp1Pct / leverage) / 100), direction: 'LONG', openAt: ts, marginSize: getMargin(capital), capitalEntry: capital };
                    signal = null;
                    continue;
                  }
                }
                ...
                if (btWaitMode === 'trend') {
                  ...
                  if ((isCompound ? capital > 0 : capital >= btUsdtPerTrade) && signal.retested && price < signal.level) {
                    openTrade = { entry: price, sl: price * (1 - (btSlPct / leverage) / 100), tp1: price * (1 - (btTp1Pct / leverage) / 100), direction: 'SHORT', openAt: ts, marginSize: getMargin(capital), capitalEntry: capital };
                    signal = null;
                    continue;
                  }
                }
        ...
        if (totalTrades) {
          ...
          const marginLabel = isCompound ? `${btMarginPct}% compound` : `$${btUsdtPerTrade.toLocaleString()}/trade`;
...
function showBtConfigMenu(bot, chatId, msgId) {
  ...
  const period = Number(getConfig('supertrendPeriod', '10'));
  const multiplier = Number(getConfig('supertrendMultiplier', '3'));
  const marginLabel = isCompoundMode ? `${marginPct}%` : 'OFF';
  const rows = [
    ...
    [{ text: `💰 Modal: $${Number(startCapital).toLocaleString()}`, callback_data: 'bt_config_capital' },
     { text: `💵 Fixed: $${Number(usdtPerTrade).toLocaleString()}`, callback_data: 'bt_config_usdtpertrade' }],
    [{ text: `🔄 Compound: ${marginLabel}`, callback_data: 'bt_config_marginpercent' }],
  ];
  if (indicator === 'ms') {
  ...
  const marginInfo = isCompoundMode ? `🔄 Compound: ${marginPct}%` : `💵 Fixed: $${Number(usdtPerTrade).toLocaleString()}`;
    ...
    async function handleCallback(query) {
      ...
      try {
        ...
        if (data === 'bt_config_marginpercent') {
          const cur = getFeatConfig('bt', 'marginPercent', '0');
          conv[chatId] = { cmd: 'bt_config', step: 'marginPercent', data: {}, promptMsgId: msgId };
          sendMenu(bot, chatId, msgId, `Compound saat ini: ${cur}% (0 = OFF, pakai fixed amount)\nMasukkan margin % dari modal (contoh: 10 = 10% per trade):`, {
            reply_markup: { inline_keyboard: [[{ text: '❌ Batal', callback_data: 'bt_config_cancel' }]] }
          });
          return { action: null };
        }
    ...
    async function handleMessage(text, chatId) {
      ...
      try {
          if (session.cmd === 'bt_config') {
            if (session.step === 'slPercent' || session.step === 'tp1Percent' || session.step === 'limit' || session.step === 'startCapital' || session.step === 'usdtPerTrade' || session.step === 'marginPercent') {
              const val = session.step === 'limit' || session.step === 'startCapital' || session.step === 'usdtPerTrade' || session.step === 'marginPercent' ? parseInt(text, 10) : parseFloat(text);
