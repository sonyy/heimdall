...
function formatSummaryMessage() {
  ...
  for (const g of Object.values(groups)) {
    ...
    const totalUsdt = (totalPnl / 100) * usdtPerTrade;
    const signU = totalUsdt >= 0 ? '+' : '';
    const avgUsdt = (avgPnl / 100) * usdtPerTrade;
    ...
    if (g.directions.length > 1) {
      for (const d of g.directions) {
        ...
        const dSign = dPnl > 0 ? '+' : '';
        const dUsdt = (dPnl / 100) * usdtPerTrade;
  ...
  const allTrades = db.prepare('SELECT id, pnl FROM backtest_trades ORDER BY id ASC').all();
  let runCap = Number(startCapital);
  for (const t of allTrades) {
    runCap += (parseFloat(t.pnl || 0) / 100) * Number(usdtPerTrade);
  }
      ...
      try {
        ...
        if (btIndicator === 'st') {
          ...
          candleLoop: for (let idx = period; idx < data.length; idx++) {
            ...
            if (capital >= btUsdtPerTrade && prevBullish !== null && !prevBullish && nowBullish && !openTrade) {
            ...
            if (capital >= btUsdtPerTrade && prevBullish !== null && prevBullish && !st.isBullish && isAligned(ts) && !openTrade) {
              const sl = price * (1 - (btSlPct / leverage) / 100);
              const tp1 = price * (1 - (btTp1Pct / leverage) / 100);
              let openTs;
              try { openTs = new Date(ts).toISOString().replace('T', ' ').slice(0, 19); } catch (e) { openTs = 'unknown'; }
            ...
            if (openTrade) {
              ...
              if (openTrade.direction === 'LONG' && lowPrice <= liq) {
                const pnl = -100;
                insTrade.run(t, tf, openTrade.entry, liq, pnl.toFixed(2), liq.toFixed(2), FEE_PCT, openTrade.sl, openTrade.tp1, null, 'LIQUIDATED', 'LONG', openTrade.openAt, closeTs);
                trades.push({ pnl, result: 'LIQUIDATED', direction: 'LONG' }); capital += (pnl / 100) * btUsdtPerTrade;
                openTrade = null;
              } else if (openTrade.direction === 'SHORT' && highPrice >= liq) {
                const pnl = -100;
                insTrade.run(t, tf, openTrade.entry, liq, pnl.toFixed(2), liq.toFixed(2), FEE_PCT, openTrade.sl, openTrade.tp1, null, 'LIQUIDATED', 'SHORT', openTrade.openAt, closeTs);
                trades.push({ pnl, result: 'LIQUIDATED', direction: 'SHORT' }); capital += (pnl / 100) * btUsdtPerTrade;
                openTrade = null;
                ...
                if (lowPrice <= openTrade.sl) {
                  ...
                  trades.push({ pnl, result: 'LOSE', direction: 'LONG' }); capital += (pnl / 100) * btUsdtPerTrade;
                  openTrade = null;
                  ...
                  trades.push({ pnl, result: 'WIN', direction: 'LONG' }); capital += (pnl / 100) * btUsdtPerTrade;
                  openTrade = null;
                }
              } else {
                if (highPrice >= openTrade.sl) {
                  ...
                  trades.push({ pnl, result: 'LOSE', direction: 'SHORT' }); capital += (pnl / 100) * btUsdtPerTrade;
                  openTrade = null;
                  ...
                  trades.push({ pnl, result: 'WIN', direction: 'SHORT' }); capital += (pnl / 100) * btUsdtPerTrade;
                  openTrade = null;
                }
              }
            }
        ...
        if (btIndicator === 'ms') {
          ...
          candleLoop: for (let idx = MS_ANALYSIS_START; idx < data.length; idx++) {
            ...
            if (openTrade) {
              ...
              if (openTrade.direction === 'LONG' && lowPrice <= liq) {
                const pnl = -100;
                insTrade.run(t, tf, openTrade.entry, liq, pnl.toFixed(2), liq.toFixed(2), FEE_PCT, openTrade.sl, openTrade.tp1, null, 'LIQUIDATED', 'LONG', openTrade.openAt, ts);
                trades.push({ pnl, result: 'LIQUIDATED', direction: 'LONG' }); capital += (pnl / 100) * btUsdtPerTrade;
                closed = true;
              } else if (openTrade.direction === 'SHORT' && highPrice >= liq) {
                const pnl = -100;
                insTrade.run(t, tf, openTrade.entry, liq, pnl.toFixed(2), liq.toFixed(2), FEE_PCT, openTrade.sl, openTrade.tp1, null, 'LIQUIDATED', 'SHORT', openTrade.openAt, ts);
                trades.push({ pnl, result: 'LIQUIDATED', direction: 'SHORT' }); capital += (pnl / 100) * btUsdtPerTrade;
                closed = true;
              } else if (openTrade.direction === 'LONG') {
                if (lowPrice <= openTrade.sl) {
            ...
            if (signal) {
              ...
              if (signal.direction === 'LONG') {
                ...
                if (btWaitMode === 'trend') {
                  ...
                  if (capital >= btUsdtPerTrade && signal.retested && price > signal.level) {
                    openTrade = { entry: price, sl: price * (1 + (btSlPct / leverage) / 100), tp1: price * (1 + (btTp1Pct / leverage) / 100), direction: 'LONG', openAt: ts };
                    signal = null;
                    continue;
                  }
                }
                ...
                if (btWaitMode === 'trend') {
                  ...
                  if (capital >= btUsdtPerTrade && signal.retested && price < signal.level) {
                    openTrade = { entry: price, sl: price * (1 - (btSlPct / leverage) / 100), tp1: price * (1 - (btTp1Pct / leverage) / 100), direction: 'SHORT', openAt: ts };
                    signal = null;
                    continue;
                  }
                }
            ...
            if (higherHigh && higherLow) {
              if (price > lastHigh) {
                if (btWaitMode === 'volume') {
                  ...
                  if (capital >= btUsdtPerTrade && volume >= avgVol * (btVolumeThreshold / 100)) {
                    openTrade = { entry: price, sl: price * (1 + (btSlPct / leverage) / 100), tp1: price * (1 + (btTp1Pct / leverage) / 100), direction: 'LONG', openAt: ts };
                    continue;
                  }
                }
        ...
        if (openTrade) {
          ...
          insTrade.run(t, tf, openTrade.entry, liquidated ? liq : lastPrice, pnl.toFixed(2), liq.toFixed(2), FEE_PCT, openTrade.sl, openTrade.tp1, openTrade.tp1Hit || null, result, openTrade.direction, openTrade.openAt, lastTs);
          trades.push({ pnl, result, direction: openTrade.direction });
          capital += (pnl / 100) * btUsdtPerTrade;
        }
        ...
        if (totalTrades) {
          ...
          const capitalWarning = capital < btUsdtPerTrade ? '\n⚠️ Modal tidak cukup untuk trade baru' : '';
