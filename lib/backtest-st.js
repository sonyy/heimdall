...
const insTrade = db.prepare(`INSERT INTO backtest_trades (ticker,timeframe,entry_price,close_price,pnl,liq_price,fee,sl_price,tp1_price,tp1_hit,result,direction,opened_at,closed_at,margin_size,capital_entry,capital_exit) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`);
  ...
  tasks.sort((a, b) => a.ticker.localeCompare(b.ticker) || tfToMinutes(a.tf) - tfToMinutes(b.tf));
  ...
  db.prepare('DELETE FROM backtest_rankings').run();
  ...
  for (let i = 0; i < tasks.length; i++) {
      ...
      try {
        ...
        if (btIndicator === 'st') {
          ...
          candleLoop: for (let idx = period; idx < data.length; idx++) {
            ...
            const nowBullish = st.isBullish;

            // Long entry: ST flip bearish→bullish
            ...
            // Short entry: ST flip bullish→bearish
            if ((isCompound ? capital > 0 : capital >= btUsdtPerTrade) && prevBullish !== null && prevBullish && !st.isBullish && !openTrade) {
              const sl = price * (1 - (btSlPct / leverage) / 100);
              const tp1 = price * (1 - (btTp1Pct / leverage) / 100);
              let openTs;
              try { openTs = new Date(ts).toISOString().replace('T', ' ').slice(0, 19); } catch (e) { openTs = 'unknown'; }
    ...
    if (bot && chatId) {
      try {
        sendMenu(bot, chatId, msgId, progressText(i + 1, tasks.length), stopMarkup());
      } catch (e) {}
    }
  }
