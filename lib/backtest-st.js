...
function formatRankingsMessage() {
  ...
  for (let i = 0; i < rankings.length; i++) {
    ...
    const wr = r.total_trades ? ((r.win_rate)).toFixed(1) : '0';
    lines.push(`${medals[i]} <b>#${i + 1}</b> — <b>+${r.profit_pct.toFixed(1)}%</b> ($${r.final_equity.toFixed(0)})`);
    lines.push(`   ${r.total_trades}T ${wr}% WR | DD: ${r.max_dd.toFixed(1)}% | ${indicator}`);
  }
  return lines.join('\n');
}
...
async function runBacktest(ticker, timeframe, bot, chatId, msgId) {
  ...
  const btMsRetestZone = Number(getFeatConfig('bt', 'msRetestZone', '2.0'));
  const btMsRrRatio = Number(getFeatConfig('bt', 'msRrRatio', '1.5'));
  const btMsMaxSlPct = Number(getFeatConfig('bt', 'msMaxSlPct', '5'));
  const btMsTrailingPct = Number(getFeatConfig('bt', 'msTrailingPct', '50')) / 100;
...
async function handleBtRun(bot, chatId, msgId, conv) {
  ...
  sendMenu(bot, chatId, null, resultText, {
    parse_mode: 'HTML',
    reply_markup: { inline_keyboard: [[{ text: '🏆 Rankings', callback_data: 'bt_rankings' }, { text: '🔙 Kembali', callback_data: 'bt_config_back' }]] }
  });
}
