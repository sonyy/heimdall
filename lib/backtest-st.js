...
function saveBacktestRanking(totalPnl, finalEquity, maxDD, totalTrades, winRate) {
  ...
  for (let i = 0; i < tasks.length; i += 5) {
    ...
    const batchResults = await Promise.all(batch.map(async ({ ticker: t, tf }) => {
      try {
        ...
        const btSlPct = tfParams.slPercent;
        const btTp1Pct = tfParams.tp1Percent;
...
function showBtPairDetail(bot, chatId, msgId, ticker) {
  ...
  if (globalIndicator === 'st' && tfs.length) {
    lines.push('');
    lines.push('<b>Per-TF SL/TP:</b>');
    for (const tf of tfs) {
      const tfSl = getConfig(`bt_slPercent_${tf}`);
      const tfTp = getConfig(`bt_tp1Percent_${tf}`);
      const slTxt = tfSl != null ? `${tfSl}%` : `${globalSl}% (global)`;
      const tpTxt = tfTp != null ? `${tfTp}%` : `${globalTp}% (global)`;
      lines.push(`  ${tf}: SL ${slTxt} / TP ${tpTxt}`);
    }
  }
  const rows = [
  ...
  if (globalIndicator === 'st' && tfs.length) {
    for (const tf of tfs) {
      rows.push([
        { text: `📉 ${tf} SL`, callback_data: `bt_pertf_sl_${ticker}_${tf}` },
        { text: `📈 ${tf} TP`, callback_data: `bt_pertf_tp_${ticker}_${tf}` },
      ]);
    }
  }
...
async function handleBtRun(bot, chatId, msgId, conv) {
  ...
  try {
    const summaryRows = db.prepare("SELECT SUM(total_trades) as tt, SUM(win) as tw, SUM(lose) as tl, SUM(total_pnl) as tp FROM backtest_summary").get();
    if (summaryRows && summaryRows.tt > 0) {
      const totalTrades = summaryRows.tt;
      const winRate = summaryRows.tw / totalTrades * 100;
      const totalPnl = summaryRows.tp;
      const allTrades = db.prepare('SELECT id, pnl, margin_size FROM backtest_trades ORDER BY id ASC').all();
      const startCap = Number(getFeatConfig('bt', 'startCapital', '1000'));
      let eq = startCap;
      let peak = startCap;
      let maxDD = 0;
      for (const t of allTrades) {
        eq += (parseFloat(t.pnl || 0) / 100) * (t.margin_size || 100);
        if (eq > peak) peak = eq;
        const dd = ((peak - eq) / peak) * 100;
        if (dd > maxDD) maxDD = dd;
      }
      saveBacktestRanking(totalPnl, eq, maxDD, totalTrades, winRate);
    }
  } catch (e) {
    console.error('Ranking save error:', e.message);
  }
  ...
  register(bot) {
    ...
    async function handleCallback(query) {
      ...
      try {
        ...
        if (data.startsWith('bt_pertf_sl_') || data.startsWith('bt_pertf_tp_')) {
          const isSl = data.startsWith('bt_pertf_sl_');
          const rest = data.replace(isSl ? 'bt_pertf_sl_' : 'bt_pertf_tp_', '');
          const lastUs = rest.lastIndexOf('_');
          const ticker = rest.slice(0, lastUs);
          const tf = rest.slice(lastUs + 1);
          const key = isSl ? `bt_slPercent_${tf}` : `bt_tp1Percent_${tf}`;
          const globalKey = isSl ? 'bt_slPercent' : 'bt_tp1Percent';
          const def = getFeatConfig('bt', globalKey, isSl ? '-2' : '1');
          const cur = getConfig(key) || def;
          conv[chatId] = {
            cmd: 'bt_pertf',
            step: isSl ? 'pertf_sl' : 'pertf_tp',
            data: { ticker, tf },
            promptMsgId: msgId,
          };
          sendMenu(bot, chatId, msgId, `${tf} ${isSl ? 'SL' : 'TP'} saat ini: ${cur}%\nMasukkan ${isSl ? 'SL' : 'TP'} baru untuk ${tf} (atau kosongkan untuk reset ke global ${def}%):`, {
            reply_markup: { inline_keyboard: [[{ text: '❌ Batal', callback_data: `bt_managepair_edit_${ticker}` }]] }
          });
          return { action: null };
        }
    ...
    async function handleMessage(text, chatId) {
      ...
      try {
        ...
        if (session.cmd === 'bt_pertf') {
          const { ticker, tf } = session.data;
          const isSl = session.step === 'pertf_sl';
          const key = isSl ? `bt_slPercent_${tf}` : `bt_tp1Percent_${tf}`;
          const val = parseFloat(text);
          if (text.trim() === '' || text.trim().toLowerCase() === 'reset') {
            db.prepare('DELETE FROM config WHERE key = ?').run(key);
          } else if (isNaN(val)) {
            sendMenu(bot, chatId, null, '❌ Masukkan angka yang valid atau kosongkan untuk reset.');
            return true;
          } else {
            upsertConfig(key, val);
          }
