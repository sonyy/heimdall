...
function formatSummaryMessage() {
  ...
  const msTxt = indicator === 'ms' ? ` | Retest: ${getFeatConfig('bt', 'msRetestZone', '1.5')}% | RR: ${getFeatConfig('bt', 'msRrRatio', '2.0')}x` : '';
...
async function runBacktest(ticker, timeframe, bot, chatId, msgId) {
  ...
  const btMsRetestZone = Number(getFeatConfig('bt', 'msRetestZone', '1.5'));
  const btMsRrRatio = Number(getFeatConfig('bt', 'msRrRatio', '2.0'));
              ...
              if (signal.direction === 'LONG') {
                ...
                if (btWaitMode === 'trend') {
                  ...
                  if ((isCompound ? capital > 0 : capital >= btUsdtPerTrade) && signal.retested && price > signal.level) {
                    ...
                    if (slDist > 0 && signal.slLevel > liqLong * 1.02) {
                ...
                if (btWaitMode === 'trend') {
                  ...
                  if ((isCompound ? capital > 0 : capital >= btUsdtPerTrade) && signal.retested && price < signal.level) {
                    ...
                    if (slDist > 0 && signal.slLevel < liqShort * 0.98) {
...
function showBtFeatureMenu(bot, chatId, msgId) {
  ...
  const text = `📊 <b>Backtest</b>\n` +
    `📉 SL: ${sl}% | 📈 TP: ${tp1}%\n` +
    `📊 Indicator: ${indicator.toUpperCase()}${waitTxt}${msTxt} | ST: ${period}×${multiplier}\n` +
...
function showBtConfigMenu(bot, chatId, msgId) {
  ...
  const rows = [
    ...
    [{ text: `⚖️ Margin Mode: ${marginModeLabel}`, callback_data: 'bt_config_marginmode' }],
    [{ text: isCompoundMode ? `🔄 Margin: ${marginPct}%` : `💵 Fixed: $${Number(usdtPerTrade).toLocaleString()}`, callback_data: isCompoundMode ? 'bt_config_marginpercent' : 'bt_config_usdtpertrade' }],
  ];
  if (indicator === 'ms') {
    ...
    rows.push([{ text: `📐 RR Ratio: ${getFeatConfig('bt', 'msRrRatio', '2.0')}x`, callback_data: 'bt_config_msrrratio' }]);
  }
  ...
  const marginInfo = isCompoundMode ? `⚖️ Mode: Percent (${marginPct}%)` : `⚖️ Mode: Fixed ($${Number(usdtPerTrade).toLocaleString()})`;
  const msInfo = indicator === 'ms' ? `\n🎯 Retest Zone: ${getFeatConfig('bt', 'msRetestZone', '1.5')}%\n📐 RR Ratio: ${getFeatConfig('bt', 'msRrRatio', '2.0')}x` : '';
  const text = `⚙️ <b>Backtest Config</b>\n\n📉 SL: ${sl}%\n📈 TP: ${tp1}%\n📊 Indicator: ${indicator.toUpperCase()} | ST: ${period}×${multiplier}\n📋 Mode: ${mode}\n🔢 Limit: ${limit}\n💰 Modal: $${Number(startCapital).toLocaleString()}\n${marginInfo}${msInfo}\n📅 ${startDate} → ${endDate}`;
  sendMenu(bot, chatId, msgId, text, {
    parse_mode: 'HTML',
    ...
    async function handleMessage(text, chatId) {
      ...
      try {
          if (session.cmd === 'bt_config') {
            if (session.step === 'slPercent' || session.step === 'tp1Percent' || session.step === 'limit' || session.step === 'startCapital' || session.step === 'usdtPerTrade' || session.step === 'marginPercent' || session.step === 'msRetestZone' || session.step === 'msRrRatio') {
