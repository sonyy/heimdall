const { db, getFeatConfig, getTfConfig, getConfig, loadPairsFor } = require('../lib/db');
const { fetchKlines, VALID_TIMEFRAMES } = require('../lib/exchange');
...
function calcSupertrend(candles, period, multiplier) {
  ...
  return { isBullish: direction === 1, wasBullish: prevDirection === 1, price: close[close.length - 1], upperBand: finalUpperBand, lowerBand: finalLowerBand };
}

module.exports = async function renderStSimulasi() {
  ...
  const cfg = {};
  for (const r of db.prepare("SELECT key, value FROM config WHERE key IN ('st_slPercent','st_tp1Percent','st_tp2Percent','st_running','st_usdtPerTrade','st_startCapital')").all()) {
    cfg[r.key] = r.value;
  }
  ...
  const startCap = Number(cfg.st_startCapital || '1000');
  const usdtPerTrade = Number(cfg.st_usdtPerTrade || '100');

  const chronoTrades = [...db.prepare('SELECT * FROM sim_trades ORDER BY id').all()];
  const capMap = {};
  let runCap = startCap;
  for (const t of chronoTrades) {
    const m = t.margin_size || usdtPerTrade;
    runCap += (parseFloat(t.pnl || 0) / 100) * m;
    capMap[t.id] = runCap;
  }
  const endCapital = runCap.toFixed(2);

  // --- Live Supertrend Status ---
  const stPairs = loadPairsFor('st_pairs');
  const stStatusLines = [];
  for (const [ticker, tfs] of Object.entries(stPairs)) {
    ...
    for (const tf of sorted) {
      try {
        ...
        if (data && data.length) {
          ...
          // Use closed candles (exclude current forming candle) — sama dengan mesin notif ST
          const closed = data.length > tfCfg.period + 2 ? data.slice(0, -1) : data;
          const st = calcSupertrend(closed, tfCfg.period, tfCfg.multiplier);
          if (tickerPrice === null) tickerPrice = st.price;
          const emoji = st.isBullish ? '🟢' : '🔴';
          const color = st.isBullish ? '#00ff00' : '#ff4444';
          // Bullish = lower band is breakout support, Bearish = upper band is breakdown resistance
          const band = st.isBullish ? st.lowerBand : st.upperBand;
          dirs.push(`<span style="color:${color}">${tf} ${emoji}</span> $${band.toFixed(2)}`);
        } else {
          dirs.push(`${tf} ⚪`);
        }
      } catch (e) {
    ...
    const priceStr = tickerPrice != null ? `$${tickerPrice.toFixed(2)}` : '-';
    stStatusLines.push(`<tr><td><strong>${ticker}</strong></td><td class="pos">${priceStr}</td><td>${dirs.join('<br>')}</td></tr>`);
  }
...
<style>
  ...
  .st-table td:last-child, .st-table th:last-child { text-align: left; }
  .st-table td:nth-child(2), .st-table th:nth-child(2) { text-align: right; }
  @media (max-width: 640px) {
    .data-table { font-size: 11px; }
    .data-table.wide-table { min-width: 900px; }
    ...
    .st-table td:nth-child(2) { font-size: 12px; }
  }
  h3 { color: #888; font-size: 14px; margin: 16px 0 8px; border-bottom: 1px solid #333; padding-bottom: 4px; text-transform: uppercase; font-family: 'Courier New', monospace; }
</style>
...
<div class="stats-bar">
  ...
  <div class="item">
    <div class="label">SL / TP / TP2</div>
    <div class="value"><span class="neg">${sl}%</span> / <span class="pos">${tp1}%</span> / <span class="pos">${tp2}%</span></div>
  </div>
  <div class="item">
    <div class="label">Modal</div>
    <div class="value" style="color:#888">$${startCap.toLocaleString()}</div>
  </div>
  <div class="item">
    <div class="label">Per Trade</div>
    <div class="value" style="color:#888">$${usdtPerTrade.toLocaleString()}</div>
  </div>
  <div class="item">
    <div class="label">Modal Akhir</div>
    <div class="value" style="color:${parseFloat(endCapital) >= startCap ? '#00ff00' : '#ff4444'}">$${Number(endCapital).toLocaleString()}</div>
  </div>
</div>

<h3>📈 Supertrend Status</h3>
${stStatusLines.length ? `
<div class="table-wrap">
<table class="data-table st-table">
  <thead>
    <tr><th>Ticker</th><th>Price</th><th>Signals</th></tr>
  </thead>
  <tbody>
    ${stStatusLines.join('')}
  </tbody>
</table>
</div>` : `<p style="color:#888;font-family:'Courier New',monospace;font-size:12px">Belum ada pair. Tambah via Telegram /menu → ST → Manage Pair.</p>`}
...
<h3>Open Trades (Notifikasi ST)</h3>
<div class="table-wrap">
<table class="data-table wide-table">
  <thead>
    <tr><th>#</th><th>Ticker</th><th>TF</th><th>Entry</th><th>SL</th><th>TP</th><th>TP2</th><th>Margin</th><th>Modal</th><th>Opened</th></tr>
  </thead>
  <tbody>
    ${openTrades.length ? openTrades.map(t => `
      <tr>
        ...
        <td style="white-space:nowrap">${t.capital_entry != null ? '<span style="color:#888">$' + fmt(t.capital_entry) + '</span> → ' : ''}${capMap[t.id] != null ? '<span style="color:' + (capMap[t.id] >= startCap ? '#00ff00' : '#ff4444') + '">$' + fmt(capMap[t.id]) + '</span>' : '-'}</td>
        <td style="color:#888">${fmtDate(t.opened_at)}</td>
      </tr>`).join('') : '<tr><td colspan="10" style="text-align:center;color:#888">Tidak ada open trade</td></tr>'}
  </tbody>
</table>
</div>

<h3>Trade History (Notifikasi ST)</h3>
<div class="table-wrap">
<table class="data-table wide-table">
  <thead>
    <tr><th>#</th><th>Ticker</th><th>TF</th><th>Entry</th><th>Close</th><th>PnL</th><th>Margin</th><th>Modal</th><th>Exit</th><th>Opened</th><th>Closed</th></tr>
  </thead>
  <tbody>
    ${closedTrades.length ? closedTrades.map(t => `
      <tr>
        ...
        <td style="color:#888">${t.margin_size != null ? '$' + fmt(t.margin_size) : '$' + usdtPerTrade}</td>
        <td style="white-space:nowrap">${t.capital_entry != null ? '<span style="color:#888">$' + fmt(t.capital_entry) + '</span> → ' : ''}${capMap[t.id] != null ? '<span style="color:' + (capMap[t.id] >= startCap ? '#00ff00' : '#ff4444') + '">$' + fmt(capMap[t.id]) + '</span>' : '-'}</td>
        <td><span class="badge ${badgeCls(t.result)}">${t.result === 'LIQUIDATED' ? 'LIQ' : t.result === 'WIN' ? 'TP' : t.result === 'LOSE' ? 'SL' : '-'}</span></td>
        <td style="color:#888">${fmtDate(t.opened_at)}</td>
