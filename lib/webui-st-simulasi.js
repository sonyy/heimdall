...
module.exports = async function renderStSimulasi() {
  ...
  for (const [ticker, tfs] of Object.entries(stPairs)) {
    ...
    for (const tf of sorted) {
      try {
        ...
        if (data && data.length) {
          ...
          dirs.push(`<span style="color:${color}">${tf} ${emoji}</span>`);
        } else {
          dirs.push(`${tf} ⚪`);
        }
      } catch (e) {
    ...
    const priceStr = tickerPrice != null ? ` <span style="color:#888">$${tickerPrice.toFixed(2)}</span>` : '';
    stStatusLines.push(`<tr><td><strong>${ticker}</strong>${priceStr}</td><td>${dirs.join(' &nbsp;│&nbsp; ')}</td></tr>`);
  }
