...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS rsi_pairs (
    ticker TEXT NOT NULL,
    timeframe TEXT NOT NULL,
    PRIMARY KEY (ticker, timeframe)
  );
  ...
  CREATE TABLE IF NOT EXISTS rsi_bt_results (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    ticker TEXT NOT NULL,
    timeframe TEXT NOT NULL,
    days INTEGER,
  ...
  CREATE TABLE IF NOT EXISTS fs_sent (
  ...
  CREATE TABLE IF NOT EXISTS fw_sent (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
...
try { db.exec("ALTER TABLE rsi_trades ADD COLUMN margin_mode TEXT NOT NULL DEFAULT 'cross'"); } catch (e) {}

try { db.exec("ALTER TABLE st_pairs ADD COLUMN notif_targets TEXT"); } catch (e) {}
try { db.exec("ALTER TABLE sent_news ADD COLUMN title TEXT"); } catch (e) {}
try { db.exec("ALTER TABLE sent_news ADD COLUMN labels TEXT"); } catch (e) {}
try { db.exec("ALTER TABLE sent_news ADD COLUMN fj_time TEXT DEFAULT ''"); } catch (e) {}
try { db.exec("ALTER TABLE sent_news ADD COLUMN sentiment TEXT DEFAULT 'neutral'"); } catch (e) {}
try { db.exec("ALTER TABLE sent_news ADD COLUMN sentiment_score INTEGER DEFAULT 0"); } catch (e) {}
...
function loadPairsFor(table) {
  ...
  if (getConfig('rsi_running') === undefined) upsertConfig('rsi_running', '0');
  if (getConfig('rsi_mode') === undefined) upsertConfig('rsi_mode', 'dry');
  if (getConfig('rsi_testnet') === undefined) upsertConfig('rsi_testnet', '1');
  if (getConfig('rsi_period') === undefined) upsertConfig('rsi_period', '14');
  if (getConfig('rsi_buyLevel') === undefined) upsertConfig('rsi_buyLevel', '30');
  if (getConfig('rsi_sellLevel') === undefined) upsertConfig('rsi_sellLevel', '70');
  if (getConfig('rsi_leverage') === undefined) upsertConfig('rsi_leverage', '5');
  if (getConfig('rsi_usdtPerTrade') === undefined) upsertConfig('rsi_usdtPerTrade', '100');
  if (getConfig('rsi_capital') === undefined) upsertConfig('rsi_capital', '1000');
  if (getConfig('rsi_marginMode') === undefined) upsertConfig('rsi_marginMode', 'cross');
  if (getConfig('rsi_slPercent') === undefined) upsertConfig('rsi_slPercent', '0');
  if (getConfig('rsi_tpPercent') === undefined) upsertConfig('rsi_tpPercent', '0');
  if (getConfig('rsi_bt_days') === undefined) upsertConfig('rsi_bt_days', '30');
  if (getConfig('rsi_trendFilter') === undefined) upsertConfig('rsi_trendFilter', '1');
  if (getConfig('rsi_trendPeriod') === undefined) upsertConfig('rsi_trendPeriod', '50');
  if (getConfig('rsi_trendLookback') === undefined) upsertConfig('rsi_trendLookback', '30');
  if (getConfig('rsi_trendTf') === undefined) upsertConfig('rsi_trendTf', '4h');

  if (getConfig('fj_enabled') === undefined) upsertConfig('fj_enabled', '1');
  if (getConfig('fj_interval') === undefined) upsertConfig('fj_interval', '30000');

if (getConfig('fs_enabled') === undefined) upsertConfig('fs_enabled', '1');
if (getConfig('fs_interval') === undefined) upsertConfig('fs_interval', '900000');

if (getConfig('fw_enabled') === undefined) upsertConfig('fw_enabled', '1');
if (getConfig('fw_interval') === undefined) upsertConfig('fw_interval', '1800000');
