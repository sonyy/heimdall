...
function parseFjTime(timeStr) {
  ...
  const now = new Date();
  // FJ renders time in browser TZ (Asia/Jakarta = UTC+7). Treat as WIB, convert to UTC.
  const d = new Date(Date.UTC(now.getFullYear(), months[m[3]] ?? now.getMonth(), parseInt(m[4]), parseInt(m[1]) - 7, parseInt(m[2])));
