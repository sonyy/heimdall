    ...
    const completion = await Promise.race([
      groq.chat.completions.create({
        messages: [
          {
            role: 'system',
            content: `You are a crypto/financial market sentiment classifier. Classify headlines by their market impact.
...
Key rules:
- Tariff reprieves, trade deal progress, sanctions easing, ceasefire = bullish (reduces uncertainty)
- Tariff escalation, trade war, new sanctions, conflict escalation = bearish (increases uncertainty)
- Rate cuts, dovish central bank, stimulus, fiscal spending = bullish
- Rate hikes, hawkish central bank, tight policy = bearish
- Economic slowdown, recession fears, mass layoffs = bearish
- Strong jobs, GDP growth, positive earnings = bullish

Examples:
- "Trump considering granting Canada tariff reprieve" → {"sentiment":"bullish","score":2}
- "US Fed cuts rates by 50bps" → {"sentiment":"bullish","score":3}
- "Bitcoin ETF sees record inflows" → {"sentiment":"bullish","score":2}
- "US imposes new tariffs on China" → {"sentiment":"bearish","score":2}
