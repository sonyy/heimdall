...
const LOOKBACK_MS = 1 * 60 * 60 * 1000; // 1 hour freshness window

const ALLOWED_LABELS = [
  'Bonds', 'US Bonds', 'Commodities', 'Energy',
];

function isBondsOrCommodities(labels) {
  return (labels || []).some(l => ALLOWED_LABELS.includes(l));
}
...
async function classifySentiment(headline) {
  ...
  try {
    ...
    const btcContext = btc ? `Current BTC: $${btc.price.toLocaleString()} (24h: ${btc.change24h > 0 ? '+' : ''}${btc.change24h?.toFixed(1)}%)` : '';
    ...
    const completion = await Promise.race([
      groq.chat.completions.create({
        messages: [
          {
            role: 'system',
            content: `You are a crypto market analyst. Classify how this Bonds/Commodities/Energy headline impacts BTC price.

Context: ${btcContext}

Rules:
- Bond yield spikes, hawkish Fed signals → bearish for BTC
- Bond yield drops, dovish Fed signals → bullish for BTC
- Energy price surges → can be inflationary → bearish for BTC
- Commodities rally → mixed, depends on macro context
- Score 1 = minimal BTC impact
- Score 2 = moderate indirect BTC impact
- Score 3 = significant BTC impact

Reply ONLY: {"sentiment": "bullish|bearish|neutral", "score": 1-3}`
          },
    ...
    const jsonMatch = response.match(/\{[\s\S]*?"sentiment"[\s\S]*?"score"[\s\S]*?\}/);
  ...
  try {
    ...
    const results = await page.evaluate(() => {
      ...
      document.querySelectorAll('.media.feedWrap').forEach(el => {
        ...
        out.push({ text: title, time, link, labels, critical: isCritical, active: isActive });
      });
      return out;
    });
    const totalItems = await page.evaluate(() => document.querySelectorAll('.media.feedWrap').length);
    console.log(`[FJ:${filterName}] ${results.length} critical/active, ${totalItems} items on page`);
    return results;
  } finally {
...
async function scrapeCriticalItems() {
  ...
  try {
    ...
    const [bonds, commodities] = await Promise.all([
      scrapeFilter(browser, 'Bonds').catch(e => { console.error('[FJ:Bonds] failed:', e.message); return []; }),
      scrapeFilter(browser, 'Commodities').catch(e => { console.error('[FJ:Commodities] failed:', e.message); return []; }),
    ]);
    const allItems = [...bonds, ...commodities];
    ...
    const filtered = allItems
      .filter(item => {
        ...
        if (!parsed || parsed.getTime() < cutoffMs) return false;
        if (!isBondsOrCommodities(item.labels)) return false;
        return true;
      })
      .sort((a, b) => {
    ...
    if (filtered.length === 0) {
      const fallbackCutoffMs = Date.now() - (2 * 60 * 60 * 1000);
      const fallback = allItems
        .filter(item => {
          ...
          seen.add(key);
          const parsed = parseFjTime(item.time);
          if (!parsed || parsed.getTime() < fallbackCutoffMs) return false;
          if (!isBondsOrCommodities(item.labels)) return false;
      ...
      if (fallback.length > 0) {
        console.log(`[FJ] No news in 1-hour window; fallback: found ${fallback.length} critical items in 2-hour window`);
...
function canonKey(text) {
  ...
  return Buffer.from(norm).toString('base64').substring(0, 64);
}

function isAlreadySent(newsId, title) {
  const row = db.prepare('SELECT 1 FROM sent_news WHERE news_id = ? OR lower(title) = lower(?)').get(newsId, (title || '').trim());
  return !!row;
}
...
module.exports = {
  register(bot, chatId) {
    ...
    async function pollTick() {
      ...
      if (scraping) { console.log('FJ: skip — previous scrape still running'); return; }
      ...
      try {
        ...
        for (const item of items) {
          const newsId = canonKey(item.text);
          if (isAlreadySent(newsId, item.text)) continue;
