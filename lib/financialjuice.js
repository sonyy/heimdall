...
function toWib(date) {
  ...
  const mon = wib.toLocaleString('en-US', { month: 'short', timeZone: 'UTC' });
  const hh = String(wib.getUTCHours()).padStart(2, '0');
  const mm = String(wib.getUTCMinutes()).padStart(2, '0');
  return `${hh}:${mm} WIB · ${dd} ${mon}`;
}

function formatWibFromFj(timeStr) {
  return toWib(parseFjTime(timeStr));
}
...
async function scrapeFilter(browser, filterName) {
  ...
  try {
    ...
    console.log(`[FJ:${filterName}] ${results.length} critical, ${prevCount} items loaded`);
    return results;
  } finally {
...
function markSent(newsId, title, labels) {
  db.prepare('INSERT INTO sent_news (news_id, title, labels) VALUES (?, ?, ?) ON CONFLICT(news_id) DO UPDATE SET title = excluded.title, labels = excluded.labels').run(newsId, title || '', labels || '');
}
...
function showFeatureMenu(bot, chatId, msgId) {
  ...
  const recentRows = db.prepare(
    "SELECT title, labels, sent_at FROM sent_news WHERE title != '' AND sent_at >= datetime('now', '-2 days') ORDER BY sent_at DESC LIMIT 8"
  ).all();

  let newsList = '';
  if (recentRows.length > 0) {
    const lines = recentRows.map((r, i) => {
      let labels = [];
      try { labels = JSON.parse(r.labels || '[]'); } catch {}
      const tagStr = labels.length ? labels.map(t => `#${t.replace(/\s+/g, '')}`).join(' ') : '';
      const parsedSent = r.sent_at ? new Date(r.sent_at + 'Z') : null;
      const wibTime = toWib(parsedSent);
    ...
    async function pollTick() {
      ...
      try {
        ...
        for (const item of items) {
          ...
          const wibTime = formatWibFromFj(item.time);
          ...
          markSent(newsId, item.text, JSON.stringify(item.labels || []));
          sentCount++;
        }
