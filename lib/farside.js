...
function buildFlowMsg(row) {
  const arrow = row.total > 0 ? '🟢' : row.total < 0 ? '🔴' : '⚪';
  const status = row.isFinal ? '✅ Final' : '⏳ Update (belum final)';
  const fundsLine = row.flows.map(f => `${f.name} ${fmtFlow(f.value)}`).join(' · ');
  const lines = [
    `📊 <b>BTC ETF Flow — ${row.label}</b>`,
    `💰 Total: ${arrow} <b>$${fmtFlow(row.total)}M</b>`,
    `🔄 Status: ${status}`,
    fundsLine ? `🏦 ${fundsLine}` : '',
  ...
  register(bot, chatId) {
    ...
    async function pollTick() {
      ...
      try {
        ...
        // Cycle pertama: seed baseline. Semua tanggal yang sudah ada di tabel
        // ditandai sebagai "sudah dilihat" TANPA kirim notif, supaya histori
        // tidak spam. Notif hanya untuk tanggal yang MUNCUL setelah ini.
        ...
        if (!hasBaseline && rows.length > 0) {
          ...
          console.log(`FS: baseline seeded — ${rows.filter(r => r.hasData).length} historical dates marked (no notif)`);
          cleanupOldFlows();
          return;
        }
