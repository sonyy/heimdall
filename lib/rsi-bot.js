...
function emaArr(values, period) {
  const ema = require('technicalindicators').EMA.calculate({ period, values });
  const offset = values.length - ema.length;
  const arr = new Array(values.length).fill(null);
  for (let i = 0; i < ema.length; i++) arr[i + offset] = ema[i];
  return arr;
}

function gatedSignal(signal, trend, trendFilter) {
  if (!signal) return null;
  if (!trendFilter || trend !== 'UP' && trend !== 'DOWN') return signal;
  if (trend === 'UP' && signal === 'LONG') return 'LONG';
  if (trend === 'DOWN' && signal === 'SHORT') return 'SHORT';
  return null;
}
...
module.exports = {
  register(bot, chatId) {
    ...
    async function getRsiSnapshot(ticker, tf) {
      ...
      const trendTf = getFeatConfig('rsi', 'trendTf', '4h');
      const trendPeriod = Number(getFeatConfig('rsi', 'trendPeriod', '50')) || 50;
      const trendLookback = Number(getFeatConfig('rsi', 'trendLookback', '60')) || 60;
      const fetchLen = Math.max(300, period + 20);
      const trendLen = Math.max(200, trendPeriod + trendLookback + 20);
      const [{ data }, { data: trendData }] = await Promise.all([
        fetchCandlesFlex(ticker, tf, fetchLen),
        fetchCandlesFlex(ticker, trendTf, trendLen),
      ]);
      ...
      const trendForming = trendData[trendData.length - 1].openTime + tfToMinutes(trendTf) * 60000 > Date.now();
      const trendClosed = trendForming ? trendData.slice(0, -1) : trendData;
      const trendCloses = trendClosed.map(c => c.close);
      const trendEma = emaArr(trendCloses, trendPeriod);
      const emaLen = trendEma.length;
      const trend =
        emaLen < trendLookback + 1 || trendEma[emaLen - 1] == null || trendEma[emaLen - 1 - trendLookback] == null
          ? 'FLAT'
          : trendEma[emaLen - 1] > trendEma[emaLen - 1 - trendLookback]
            ? 'UP'
            : 'DOWN';
      return {
    ...
    async function manageOpen(trade, snapshot, cfg) {
      ...
      const trendFilter = getFeatConfig('rsi', 'trendFilter', '1') === '1';
      const signal = gatedSignal(detectSignal(snapshot.prevRsi, snapshot.currRsi, Number(cfg.buyLevel), Number(cfg.sellLevel)), snapshot.trend || 'FLAT', trendFilter);
...
async function runRsiBacktest(ticker, timeframe, days, overrides = {}) {
  ...
  const rsiArr = RSI.calculate({ period, values: closes });
  const trendPeriod = cfg('trendPeriod', '50') || 50;
  const trendFilter = cfgStr('trendFilter', '1') === '1';
  const trendLookback = cfg('trendLookback', '30') || 30;
  const trendTf = cfgStr('trendTf', '4h');
  let trendPerBar = null;
  if (trendFilter) {
    const warmupMs = (trendPeriod + trendLookback + 20) * tfToMinutes(trendTf) * 60000;
    const trendLimit = Math.ceil(days * 24 * 60 / tfToMinutes(trendTf)) + trendPeriod + trendLookback + 40;
    const { data: trendRaw } = await fetchKlinesRangeFlex(ticker, trendTf, startTime - warmupMs, endTime, trendLimit);
    const trendData = trendRaw[trendRaw.length - 1].openTime + tfToMinutes(trendTf) * 60000 > Date.now()
      ? trendRaw.slice(0, -1) : trendRaw;
    const trendCloses = trendData.map(c => c.close);
    const trendTimes = trendData.map(c => c.openTime);
    const trendEma = emaArr(trendCloses, trendPeriod);
    trendPerBar = new Array(data.length).fill('FLAT');
    const tfMs = tfToMinutes(timeframe) * 60000;
    const trendMs = tfToMinutes(trendTf) * 60000;
    let tIdx = -1;
    for (let i = 0; i < data.length; i++) {
      const t = data[i].openTime + tfMs; // keputusan diambil saat candle i close
      while (tIdx + 1 < trendTimes.length && trendTimes[tIdx + 1] + trendMs <= t) tIdx++;
      const eNow = tIdx >= 0 ? trendEma[tIdx] : null;
      const ePrev = tIdx >= trendLookback ? trendEma[tIdx - trendLookback] : null;
      trendPerBar[i] = (eNow != null && ePrev != null) ? (eNow > ePrev ? 'UP' : 'DOWN') : 'FLAT';
    }
  }
  ...
  for (let i = period; i < data.length; i++) {
    ...
    const trend = trendPerBar ? trendPerBar[i] : 'FLAT';
    const signal = gatedSignal(detectSignal(prevRsi, currRsi, buyLevel, sellLevel), trend, trendFilter);
  ...
  const configJson = JSON.stringify({ period, buyLevel, sellLevel, leverage, usdtPerTrade, capital, marginMode, slPercent, tpPercent, trendFilter, trendPeriod, trendLookback, trendTf });
  try {
