...
module.exports = function renderBacktestSt() {
  ...
  const lastTrade = db.prepare('SELECT capital_exit FROM backtest_trades ORDER BY id DESC LIMIT 1').get();
  agg.endCapital = lastTrade && lastTrade.capital_exit != null ? Number(lastTrade.capital_exit).toFixed(2) : null;
