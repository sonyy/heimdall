...
module.exports = function renderBacktestSt() {
  const cfg = {};
  for (const r of db.prepare("SELECT key, value FROM config WHERE key IN ('bt_slPercent','bt_tp1Percent','bt_mode','bt_limit','bt_startDate','bt_endDate','bt_indicator','bt_waitMode','bt_startCapital','bt_usdtPerTrade','bt_marginPercent','bt_marginMode') OR key LIKE 'bt_leverage_%'").all()) {
    cfg[r.key] = r.value;
  }
...
<table class="data-table">
  <thead>
    <tr><th>#</th><th>Ticker</th><th>TF</th><th>Dir</th><th>Entry</th><th>Close</th><th>SL</th><th>TP</th><th>Liq</th><th>Fee</th><th>PnL</th><th>PnL (USDT)</th><th>Margin</th><th>Modal</th><th>Result</th><th>Opened</th><th>Closed</th></tr>
  </thead>
  <tbody>
    ${trades.length ? trades.map(t => `
      ...
      </tr>`).join('')       : '<tr><td colspan="17" class="empty">Belum ada trade history — jalankan ▶️ Run Backtest</td></tr>'}
  </tbody>
</table>
</div>`;
};
