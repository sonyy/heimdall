...
module.exports = function renderBacktestSt() {
  const cfg = {};
  for (const r of db.prepare("SELECT key, value FROM config WHERE key IN ('bt_slPercent','bt_tp1Percent','bt_mode','bt_limit','bt_startDate','bt_endDate','bt_indicator','bt_waitMode','bt_startCapital','bt_usdtPerTrade','bt_marginPercent','bt_marginMode','bt_msRetestZone','bt_msRrRatio','bt_msMaxSlPct','bt_msTrailingPct') OR key LIKE 'bt_leverage_%'").all()) {
    cfg[r.key] = r.value;
  }
