...
module.exports = function renderBacktestSt() {
  ...
  const trades = db.prepare('SELECT * FROM backtest_trades ORDER BY id DESC LIMIT 50').all();

  // --- Config vars (declared BEFORE agg block uses them) ---
  ...
  if (allTrades.length) {
    ...
    // USD PnL per trade = (pnl% / 100) * margin_size
    const totalUsdt = allTrades.reduce((s, t) => s + (parseFloat(t.pnl || 0) / 100 * (t.margin_size || Number(usdtPerTrade))), 0);
    agg.totalUsdt = totalUsdt.toFixed(2);
    agg.avgUsdt = allTrades.length > 0 ? (totalUsdt / allTrades.length).toFixed(2) : '0.00';
    // Portfolio equity = start capital + sum of all USD PnL (consistent across all pairs)
    agg.endCapital = (startCap + totalUsdt).toFixed(2);
  } else {
    agg.endCapital = startCap.toFixed(2);
  }
  ...
  const rankings = rankingsRaw.map(row => {
    ...
    if (startStr && endStr) {
      ...
      if (!isNaN(start) && !isNaN(end)) {
        ...
        diffDays = Math.max(1, Math.floor(diffTime / (1000 * 60 * 60 * 24)));
  ...
  <tbody>
    ${trades.length ? trades.map(t => `
      <tr>
        ...
        <td style="white-space:nowrap">${t.capital_entry != null ? '<span style="color:#888">$' + fmt(t.capital_entry) + '</span> → ' : ''}${t.capital_exit != null ? '<span style="color:' + (t.capital_exit >= startCap ? '#00ff00' : '#ff4444') + '">$' + fmt(t.capital_exit) + '</span>' : '-'}</td>
