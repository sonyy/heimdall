...
module.exports = function renderBacktestSt() {
  const cfg = {};
  for (const r of db.prepare("SELECT key, value FROM config WHERE key IN ('bt_slPercent','bt_tp1Percent','bt_mode','bt_limit','bt_startDate','bt_endDate','bt_indicator','bt_waitMode','bt_startCapital','bt_usdtPerTrade') OR key LIKE 'bt_leverage_%'").all()) {
    cfg[r.key] = r.value;
  }
...
<table class="data-table">
  <thead>
    <tr><th>#</th><th>Ticker</th><th>TF</th><th>Dir</th><th>Entry</th><th>Close</th><th>SL</th><th>TP</th><th>Liq</th><th>Fee</th><th>PnL</th><th>PnL (USDT)</th><th>Capital</th><th>Result</th><th>Opened</th><th>Closed</th></tr>
  </thead>
  <tbody>
    ${trades.length ? trades.map(t => `
      <tr>
        ...
        <td style="color:${capMap[t.id] != null && capMap[t.id] >= startCap ? '#00ff00' : '#ff4444'}">${capMap[t.id] != null ? '$' + fmt(capMap[t.id]) : '-'}</td>
