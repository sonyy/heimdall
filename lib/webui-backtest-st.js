...
module.exports = function renderBacktestSt() {
  const cfg = {};
  for (const r of db.prepare("SELECT key, value FROM config WHERE key IN ('bt_slPercent','bt_tp1Percent','bt_mode','bt_limit','bt_startDate','bt_endDate','bt_indicator','bt_waitMode','bt_startCapital','bt_usdtPerTrade','bt_marginPercent') OR key LIKE 'bt_leverage_%'").all()) {
    cfg[r.key] = r.value;
  }
  ...
  const marginPct = cfg.bt_marginPercent || '0';
...
<div class="stats-bar">
  <div class="stats-section">
    ...
    <div class="section-items">
      ...
      <div class="item">
        <div class="label">${isCompoundMode ? 'Compound' : 'Per Trade'}</div>
...
<table class="data-table">
  <thead>
    <tr><th>#</th><th>Ticker</th><th>TF</th><th>Dir</th><th>Entry</th><th>Close</th><th>SL</th><th>TP</th><th>Liq</th><th>Fee</th><th>PnL</th><th>PnL (USDT)</th><th>Modal</th><th>Result</th><th>Opened</th><th>Closed</th></tr>
  </thead>
  <tbody>
    ${trades.length ? trades.map(t => `
      <tr>
        ...
        <td class="${pnlCls(t.pnl)}">${t.pnl != null ? (parseFloat(t.pnl) / 100 * (t.margin_size || Number(usdtPerTrade))).toFixed(2) : '-'}</td>
