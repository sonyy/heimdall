...
module.exports = function renderBacktestSt() {
  ...
  const summary = db.prepare('SELECT ticker, timeframe, SUM(total_trades) as tt, SUM(win) as tw, SUM(lose) as tl, SUM(total_pnl) as tp, MAX(win_rate) as wr FROM backtest_summary GROUP BY ticker, timeframe ORDER BY SUM(total_pnl) DESC').all();
...
<style>
  ...
  .stats-section.hasil { border-left: none; padding-left: 0; margin-top: 20px; }
...
<table class="data-table">
  <thead>
    <tr><th>#</th><th>Ticker</th><th>TF</th><th>Dir</th><th class="mobile-hide">Entry</th><th class="mobile-hide">Close</th><th class="mobile-hide">SL</th><th class="mobile-hide">TP</th><th class="mobile-hide">Liq</th><th class="mobile-hide">Fee</th><th>PnL</th><th>Margin</th><th class="mobile-hide">Modal</th><th>Exit</th><th>Opened</th><th class="mobile-hide">Closed</th></tr>
  </thead>
  <tbody>
    ${trades.length ? trades.map(t => `
      <tr>
        ...
        <td class="mobile-hide" style="font-size:11px;white-space:nowrap">${t.capital_entry != null ? '<span style="color:#888">$' + fmt(t.capital_entry) + '</span> → ' : ''}${capMap[t.id] != null ? '<span style="color:' + (capMap[t.id] >= startCap ? '#00ff00' : '#ff4444') + '">$' + fmt(capMap[t.id]) + '</span>' : '-'}</td>
