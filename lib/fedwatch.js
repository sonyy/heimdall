...
// ─── Sumber data ─────────────────────────────────────────────────────────────
// CME memblokir IP server secara langsung (403 "blocked due to suspected web
// scraping"), jadi fetch settlement lewat jina reader proxy (GET-friendly).
...
// ─── Orchestrasi fetch + hitung (dipanggil tiap poll) ────────────────────────
...
function buildFwMsg(data) {
  ...
  if (first) {
    const rows = Object.entries(first.probs).sort((a, b) => b[1] - a[1]);
