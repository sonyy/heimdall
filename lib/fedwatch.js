...
// ─── Sumber data ─────────────────────────────────────────────────────────────
// SUMBER UTAMA: kolom "NOW" tool FedWatch (QuikStrike) langsung dari website.
...
// FALLBACK: hitung ulang dari CME settlement (EOD) yg di-fetch lewat jina
// reader proxy — dipakai bila scrape live gagal.
// FRED (EFFR & target range) bisa diakses langsung.
...
async function fetchLiveFedWatch(firstOnly) {
  ...
  try {
    ...
    const tabs = await page.evaluate(() => {
      const ul = document.querySelector('ul.qs-htabs.inner-tabs');
      if (!ul) return [];
      return [...ul.querySelectorAll('li')].map((li) => (li.innerText || '').trim()).filter(Boolean);
    });

const meetings = [];
    const seen = new Set();
    const tabsUl = page.locator('ul.qs-htabs.inner-tabs');
    const targets = firstOnly ? tabs.slice(0, 1) : tabs;
    for (let i = 0; i < targets.length; i++) {
      if (i > 0) {
        try {
          // Klik real (bukan sintetik): handler tool hanya merespons event mouse asli
          await tabsUl.getByText(tabs[i], { exact: true }).first().click({ timeout: 15000 });
          await page.waitForTimeout(8000);
        } catch (e) {
          console.warn(`FW: tab ${tabs[i]} gagal diklik: ${e.message.slice(0, 120)}`);
          continue;
        }
      }
      const v = await parseFedWatchMeeting(page).catch(() => null);
      if (!v || !v.date || !v.ranges.length) continue;
      ...
      meetings.push({ date: iso, contract: v.contract || '', probs, currentLabel });
    }

    if (!meetings.length) throw new Error('tidak ada data meeting terbaca dari tool');
    meetings.sort((a, b) => a.date.localeCompare(b.date));
    return { meetings, targetRange: meetings[0].currentLabel || '' };
  } finally {
    await browser.close().catch(() => {});
  }
}

async function fetchSettlementFallback() {
  const td = new Date(Date.now() - 86400000);
  while (td.getUTCDay() === 0 || td.getUTCDay() === 6) td.setUTCDate(td.getUTCDate() - 1);
  const tradeDateStr = fmtTradeDate(td);

  const [cme, effr] = await Promise.all([
    fetchCmeSettlements(tradeDateStr),
    fetchFredSeries('EFFR'),
  ]);

  const today = new Date();
  const upcoming = FOMC_MEETINGS.filter((m) => m >= today);
  if (upcoming.length <= 3) {
    console.warn(`FW: kalender FOMC hampir habis (${upcoming.length} tersisa) — update FOMC_MEETINGS di lib/fedwatch.js`);
  }
  if (upcoming.length === 0) throw new Error('kalender FOMC habis');

  const calc = calculateProbabilities(cme.settlements, upcoming, effr);
  return { effr, tradeDate: cme.tradeDate, targetRange: calc.targetRange, meetings: calc.results, fetchedAt: new Date().toISOString() };
}

// ─── Orchestrasi fetch (dipanggil tiap poll) ─────────────────────────────────
async function fetchFedWatchData(firstOnly = false) {
  let live;
  try {
    live = await fetchLiveFedWatch(firstOnly);
  } catch (e) {
    console.warn(`FW: scrape live gagal (${e.message.slice(0, 140)}) — fallback ke settlement`);
  }
  if (live) {
    let effr = null;
  ...
  return fetchSettlementFallback();
}
...
function nextFomcLine() {
  ...
  const dateStr = fmtDateLabel(next.toISOString().slice(0, 10));
  if (days <= 0) return `⏭ FOMC: ${dateStr} (hari ini)`;
  if (days === 1) return `⏭ FOMC: ${dateStr} (besok)`;
  return `⏭ FOMC: ${dateStr} (${days} hari lagi)`;
}
  ...
  register(bot, chatId) {
    ...
    async function pollTick() {
      ...
      try {
        ...
        if (last.snapshot_json === fp) {
          console.log('FW: tidak ada perubahan probabilitas');
          return;
        }
    ...
    async function showNow(chatId) {
      ...
      try {
        const data = await fetchFedWatchData(true);
        const msg = buildFwMsg(data);
        await bot.sendMessage(chatId, msg, {
          parse_mode: 'HTML',
          disable_web_page_preview: true,
          reply_markup: { inline_keyboard: [[{ text: '⚙️ Menu', callback_data: 'fw_mainback' }]] }
        }).catch(() => {});
      } catch (e) {
