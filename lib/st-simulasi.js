...
function openSimTrade(ticker, timeframe, price, slPrice, tp1Price, tp2Price, signal) {
  try {
    ...
    console.log(`ST TRADE OPEN: ${ticker} ${timeframe} @ $${price} (SL: $${slPrice.toFixed(2)}, TP: $${tp1Price.toFixed(2)}, TP2: $${tp2Price.toFixed(2)})`);
        ...
        try {
          ...
          if (prevAligned !== undefined && !prevAligned && nowAligned && stRunning === '1') {
            ...
            if (!existing) {
              ...
              sendNotif(bot, chatId, ticker, `🟢 <b>ST OPEN</b>\n${ticker} @ $${price}\n${allState}\nSL: $${slPrice} | TP: $${tp1Price} | TP2: $${tp2Price}`, { parse_mode: 'HTML' });
