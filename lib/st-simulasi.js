...
function showTfSelector(bot, chatId, ticker, preselected, editMsgId) {
  ...
  if (editMsgId) {
    return bot.editMessageText(lines.join('\n'), { chat_id: chatId, message_id: editMsgId, ...opts }).catch(e => {
      console.error(`TF editMessageText failed (${ticker}, msg=${editMsgId}): ${e.message}`);
        ...
        try {
          ...
          if (prevAligned !== undefined && !prevAligned && nowAligned && stRunning === '1') {
            ...
            if (!existing) {
              ...
              sendNotif(bot, chatId, ticker, `🟢 <b>ST OPEN</b>\n${ticker} @ $${price}\n${allState}\nSL: $${slPrice} | TP1: $${tp1Price} | TP2: $${tp2Price}`, { parse_mode: 'HTML' });
      ...
      for (const t of openTrades) {
        try {
          ...
          if (price >= t.tp1_price && !t.tp1_hit) {
            db.prepare('UPDATE sim_trades SET tp1_hit=? WHERE id=?').run(t.tp1_price, t.id);
            sendNotif(bot, chatId, t.ticker, `ℹ️ <b>ST TP1 HIT</b>\n${t.ticker} #${t.id} @ $${price}`, { parse_mode: 'HTML' });
          }
          if (price >= t.tp2_price) {
            if (!t.tp1_hit && price >= t.tp1_price) {
              db.prepare('UPDATE sim_trades SET tp1_hit=? WHERE id=?').run(t.tp1_price, t.id);
