...
```bash
git clone git@github.com:sonny/heimdall.git
cd heimdall
npm install
...
# Or manually
opencode run "clone https://github.com/sonny/heimdall, install deps, configure .env from .env.example, start index.js and webui-sim.js"
```
...
The backtest leaderboard now shows:
- **Daily %** - Profit per day (total profit ÷ backtest range days)
- **Range** - Start → end dates of backtest period
- **Sorting** - Based on daily profit % (highest first)
- **Full Leaderboard** - Ranks configs by performance efficiency

Refresh the Web UI tab to see updated rankings after running backtests.
